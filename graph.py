...
def main():
  ...
  if arg in ["mhLinRegrOnce", "mhLinRegr", "smcLinRegr", "rmsmcLinRegr", "pmmhLinRegr", "smc2LinRegr"]:
    ...
    axs1.set_title('Linear regression - Metropolis Hastings')
  if arg in ["inviLinRegr", "bbviLinRegr", "bbviDefaultLinRegr"]:
    mu_mean = data[0][0]
    mu_std  = data[0][1]
    c_mean  = data[1][0]
    c_std   = data[1][1]
  ...
  if arg in ["mhHMM", "smcHMM", "rmsmcHMM", "pmmhHMM", "smc2HMM"]:
    ...
    plt.show()
  if arg in ["bbviHMM", "bbviDefaultHMM", "inviHMM"]:
  ...
  if arg == "simLDA":
    ...
    ax.bar(ws, freqs)
  if arg in ["mhLDA", "smcLDA", "rmsmcLDA", "pmmhLDA", "smc2LDA"]:
  ...
  if arg in ["bbviLDA", "bbviDefaultLDA", "inviLDA"]:
    ws       = ['DNA', 'evolution', 'parsing', 'phonology']
    topic_ps = data[0]
    topic_0s = data[1]
    topic_1s = data[2]
