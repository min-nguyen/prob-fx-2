...
bbvi :: forall env a. (Show (Env env))
  ...
  -> Env env                            -- ^ model environment (containing only observed data)
  -> Model env [ObsRW env, Dist] a      -- ^ guide
  -> Sampler DTrace
...
bbviInternal :: (LastMember (Lift Sampler) fs, Show (Env env))
  ...
  -> DTrace                                 -- ^ guide initial proposals
  -> Prog fs DTrace
...
bbviStep :: (LastMember (Lift Sampler) fs, Show (Env env))
  ...
  -> DTrace                                -- ^ guide proposal distributions (Q(λ))
...
bbviStep timestep num_samples model model_env guide proposals = do
  ...
  (_               , model_logWs)
      <- Util.unzip3 <$> mapM ((lift . handleModel model) . (model_env <>)) guide_envs
