...
guidedLoop num_timesteps num_samples guide execg model execm guideParams_0 = do
  let guidedStep params = do
        let exec = do {- | Execute the guide Q under a provided set of proposal distributions Q(λ), producing:
                          1. The output environment of latent variables X=x generated by @Sample@s and @Param@s
                            (non-learnable and learnable dists)
                                x ~ Q(X; λ),         where Q can be fixed or learnable
                          2. The total log-weight of latent variables X=x:
                      ...
                      -- | Execute the model P(X, Y) under the output guide environment X, accumulating the total model log-weight:
                      (_        , model_w)       <- call (execm env model )
                      -- | Compute total log-importance-weight, log(P(X, Y)) - log(Q(X; λ))
                      let w     =  model_w - guide_w
                      pure (δλ, w)
        {- | Execute the guide and model for L iterations, accumulating:
              - the total (guide - model) log-weight: log(P(X=x, Y=y)) - log(Q(X = x; λ))
              - the gradient log-pdfs of the guide: δlog(Q(X = x; λ))
        -}
        (δλs, ws) <- unzip <$> replicateM num_samples exec
        {-  Compute an estimate of the ELBO gradient: E[δelbo] and Update the parameters λ of the guide Q(λ) -}
        call (GradUpdate ws δλs params)

  foldr (>=>) pure [guidedStep | t <- [1 .. num_timesteps]] guideParams_0
