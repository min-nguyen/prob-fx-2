...
viStep num_samples hdlGuide hdlModel guide model  params = do
  ...
  (((_, envs), guide_ρs), δλs) <- Util.unzip4 <$> replicateM num_samples (call (hdlGuide guide params))
  ...
  params'  <- call (GradDescent ρs δλs params)
  -- liftPutStrLn (show modelParams')
  pure params'

hdlGuideModel ::  (Members '[Sampler] fs)
  => GuideHandler env es1 a -> ModelHandler env es2 b -> VIGuide env es1 a -> VIModel env es2 b
  -> ParamTrace                   -- ^ guide parameters λ_t
  -> Prog fs (GradTrace, LogP)    -- ^ next guide parameters λ_{t+1}
hdlGuideModel hdlGuide hdlModel guide model params = do
  -- | Execute the guide X ~ Q(X; λ) for (L) iterations
