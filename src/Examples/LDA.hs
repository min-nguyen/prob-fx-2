...
import Env
import Inference.SIM as Simulate ( simulate )
...
simLDA = do
  let n_words = 100
      env_in = #θ := [[0.5, 0.5]] <:>
               ...
               #w := [] <:> eNil
  (words, env_out) <- Simulate.simulate (topicModel vocab 2 n_words) env_in
  return words
...
mhLDA  = do
  ...
  env_mh_outs <- MH.mhTopLevel 5000 (topicModel vocab n_topics n_words) env_mh_in (#φ ⋮ #θ ⋮ ONil)
