...
data ErasedPrimDist where
  ErasedPrimDist :: PrimDist d a => d -> ErasedPrimDist
...
singleton :: (Typeable v, PrimDist d v) => DKey v -> d -> DTrace
singleton k x = Node k x Leaf Leaf

-- | Lookup an entry
-- lookup :: forall a b d. (Eq a, Typeable a, PrimDist d a)
--   => DKey a -> DTrace -> Maybe d
-- lookup kx = go
--   where
--     go Leaf = Nothing
