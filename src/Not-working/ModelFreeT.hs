...
type Model s a = FreeT Dist (State (Record (Maybes s))) a
...
access :: Getting a (Maybes s :& Field Identity) a
...
runModelFree :: Model s a -> State (Record (Maybes s)) a
...
runModelState :: State (Record (Maybes s)) a -> Record (Maybes s) 
              -> (a, Record (Maybes s))
